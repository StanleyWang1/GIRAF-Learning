...
class MabWorker:
    ...
    def start(self) -> None:
        ...
        self.reader = threading.Thread(
            target=self._read_output, name="mab-output", daemon=True
        )
        self.reader.start()
    ...
    def command(self, roll: float, pitch: float, boom: float) -> None:
        ...
        try:
            ...
            process.stdin.write(
                f"SET {values[0]:.9g} {values[1]:.9g} {values[2]:.9g}\n"
            )
            process.stdin.flush()
...
def child_main(watchdog: float) -> int:
    ...
    result = 0
    try:
        from giraf.drivers.mab import motor_connect, motor_disconnect, motor_drive
        ...
        while True:
            if selector.select(timeout=0.05):
                ...
                last_command is not None and time.monotonic() - last_command > watchdog
            ):
        ...
        if candle is not None:
            ...
            except BaseException as exc:  # noqa: BLE001 - shutdown must be reported
