...
def sample_temperatures(path: Path, stopped: threading.Event) -> None:
    ...
    with path.open('x') as output:
        while True:
            ...
            output.write(json.dumps({'time': now(), 'temperatures_c': values,
                                     'read_errors': errors}) + '\n')
            output.flush()
            if stopped.wait(2):
                break
...
def main() -> int:
    ...
    command = ['gdb', '-nx', '-q', '-batch', '-x', str(debugger), '--args',
               sys.executable, '-m', 'giraf.learning.train_cli', *sys.argv[1:]]
    environment = os.environ.copy()
    environment.setdefault('WANDB_CONSOLE', 'off')
    status = {'state': 'starting', 'started_at': now(), 'monitor_pid': os.getpid(),
              'command': command, 'affinity': sorted(os.sched_getaffinity(0)),
    ...
    sampler = threading.Thread(target=sample_temperatures,
                               args=(args.output_dir / 'cpu_temperatures.jsonl', stopped),
                               daemon=True)
    sampler.start()
    try:
        with (args.output_dir / 'train.log').open('x') as log:
            print(f'[MONITOR] {args.output_dir}; temperature sampling every 2 seconds', flush=True)
            ...
            status.update(state='completed' if exit_code == 0 else 'failed',
